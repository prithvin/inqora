	...
	callAJAX ("GET", "/posting/getcomments", {PostId: postid}, function (data) {
		...
		if (data.length > 3) {
			...
			for (var x = 0; x < data.length - 3; x++) 
				arr.push(data[x]);
			var prevcomments = $("<a>").attr("href", "#").html("<i class='fa fa-cubes'></i> View Previous Comments").attr("data-array", JSON.stringify(arr).toString()).appendTo(panel).attr("name", postid);
			$(prevcomments).on("click", function (ev) {
				ev.preventDefault();
		...
		for (var x = start; x < data.length; x++) {
			...
			var tably = $("<table>").css("width", "100%").appendTo(divvy);
				...
				var ul1 = $("<div>").css("margin-top", "5px").appendTo(divvy);
					...
					$(ul1).append(" - ");
					var lifirst = $("<span>").appendTo(ul1).attr("id", postid + "-" + x + "-upvote").attr("name", postid).attr("value", x);
						var linkup = $("<a>").addClass(upvoteclass).attr("href" , "").html("<i class='fa fa-arrow-up'></i> Upvote").appendTo(lifirst);
					$(ul1).append(" - ");
					var lithird = $("<span>").appendTo(ul1).attr("id", postid + "-" + x + "-downvote").attr("name", postid).attr("value", x);
...
$(".sharepost").on("click", function (ev) {
	ev.preventDefault();
});
...
function createPost(postid, maindiv, appendafter) {
	callAJAX ("GET", "/posting/getpost", {PostId: postid}, function (data) {
		...
		var menu = $("<div>").addClass("pure-menu pure-menu-open pure-menu-horizontal menu").appendTo(section);
			...
			var upvote = $("<li>").addClass("mostmenu").appendTo(ul1).attr("id" ,postid + "-upvote").attr("name", postid);
			...
			var downvote = $("<li>").addClass("mostmenu").appendTo(ul1).attr("id" ,postid + "-downvote").attr("name", postid);
			...
			var commentpost = $("<li>").addClass("mostmenu commentpost").attr("id", "commentpost-" + postid).attr("name",postid).appendTo(ul1);
				...
				$(commentpost).on("click", function (ev) {
					ev.preventDefault();
