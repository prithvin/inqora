...
grunt.tasks = function(tasks, options, done) {
  ...
  verbose.header('Initializing').writeflags(option.flags(), 'Command-line options');
	//标准化任务格式，如果没有任务就使用default任务,并且确保任务是数组，这样可以多任务
  // Determine and output which tasks will be run.
  var tasksSpecified = tasks && tasks.length > 0;
  tasks = task.parseArgs([tasksSpecified ? tasks : 'default']);
	//加载命令函数（实际上就是放在../../task/所有的js），并把函数纪律到数组中.并且价值配置config文件
  // Initialize tasks.
  task.init(tasks);
  ...
  //根据用户输入的命令任务，提取系统对于的任务放入执行队列中，例如输入任务为concat,找系统对于的concat函数，有则调用否则不调用
